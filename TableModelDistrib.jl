# Parallel table-model generator: mirrors `TableModel.jl` (same model, grids, FITS layout).
# Spectrum evaluation uses `Distributed.pmap` so each worker is a separate process (safe for
# non-thread-safe backends). Only the main process opens/writes the FITS file.

using Distributed, ProgressMeter

# --- worker pool: set TABLE_MODEL_WORKERS=8 in the environment to override ---
const TABLE_MODEL_WORKERS = parse(Int, get(ENV, "TABLE_MODEL_WORKERS", string(max(1, Sys.CPU_THREADS - 1))))
if nworkers() < TABLE_MODEL_WORKERS
    proj = Base.active_project()
    nw = TABLE_MODEL_WORKERS - nworkers()
    if proj === nothing
        addprocs(nw)
    else
...
# Absolute path so `include` works the same on every worker process.
const _GRADUS_FILE = abspath(joinpath(@__DIR__, "gradus-lamp-post.jl"))

@everywhere using SpectralFitting, XSPECModels, Relxill, Warmabs
@everywhere include($(_GRADUS_FILE))

@everywhere function _tablemodel_worker_spectrum(ps, free_param_symbols, Energies, model)
    ...
    return invokemodel(Energies, local_model).parent[:, 1]
end

using CFITSIO

# ─── Match `TableModel.jl` (edit here or in TableModel.jl and keep in sync) ───
...
nrow = length(iter_params)
nb = max(1, nrow ÷ max(4 * nworkers(), 1))
println("Distributed: $(nworkers()) workers, $nrow spectra (batch_size≈$nb). pmap …")
flush(stdout)

all_spectra = pmap(
    ps -> _tablemodel_worker_spectrum(ps, free_param_symbols, Energies, model),
    iter_params;
    batch_size = nb,
)

@showprogress "Writing FITS " for j in eachindex(all_spectra)
    fits_write_col(f, 2, j, 1, all_spectra[j])
end
...
println("Done! Table written to: $Out_path")
